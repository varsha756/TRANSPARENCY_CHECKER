...
def _call_gemini(system_prompt: str, user_content: str, max_tokens: int = 2048) -> str:
    ...
    if not response.text:
        ...
        raise ValueError(
            ...
            f"possibly blocked by safety filters or truncated)"
        )
...
def analyze_report_with_ai(extracted_text: str) -> dict:
    ...
    if not GOOGLE_API_KEY:
        return {
            ...
            "red_flags": "AI analysis unavailable: missing GOOGLE_API_KEY",
    ...
    try:
        ...
        return {
            "transparency_score": 0,
            "red_flags": f"AI analysis failed: {e}",
            "summary": "The AI analysis service could not be reached or returned an unexpected response.",
        }


def analyze_money_usage(org_name: str, extracted_text: str, donation_amount: float, donation_count: int) -> dict:
    if not GOOGLE_API_KEY:
        return {"usage_summary": "AI analysis unavailable: missing GOOGLE_API_KEY."}
    ...
    try:
        ...
        return {"usage_summary": f"AI analysis failed: {e}"}